...
class Board():
    ...
    def pureSpaceCheck(self,space):
        if self.values[space] != "-":
...
def ticTacToe(): #Main Tic Tac Toe Game
    ...
    while winCon == 0:
        ...
        if space in range(9): #Needed to assure the value as a int
            check = newBoard.takeTurn(turnCounter,space) #Apply the takeTurn method from Board class
            if check == False:
                pass
            else:
...
def ticTacToeSquared(): #New addition, tic tac toes in tic tac toe
    ...
    while winCon == 0:
        ...
        checkSpot = t2Board.pureSpaceCheck(choice)
        if checkSpot == True:
            ...
            if choice in range(9):#Similar to regular tic tac toe
                if spaceWinner == "X":
                    check = t2Board.takeTurn(0,choice) #Takes the board space for x if winner
                elif spaceWinner == "O":
                    check = t2Board.takeTurn(1,choice) #Takes the board space for O if winner
                if check == False:
                    pass
                else:
...
while True:
    ...
    if choice == "Goodbye": #Break condition
        ...
        while gameEnd == False: #Game loop
            ...
            elif gameChoice != "TTT" and gameChoice != "T2": #Covers errors
